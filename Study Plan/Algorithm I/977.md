...
### 方法二：双指针
`left` 指针初始时指向 `0`，`right` 指针初始时指向 `nums.length - 1`，当 `left <= right`时，`leftSquareNum` 记录 `nums[left]` 的平方，`rightSquareNum` 记录 `nums[right]` 的平方。比较 `leftSquareNum` 与 `rightSquareNum` 将二者中的最大值逆序存入结果数组。
