...
进行 `set` 操作时，将二元组插入到 `key` 对应的 `List` 的末尾（`set` 操作中的时间戳 `timestamp` 都是严格递增的）。

进行 `get` 操作时，先从哈希表中获取到 `key` 对应的 `List`，若哈希表不存在该 `key`，则直接返回 `""`，然后在该 `List` 中二分查找最后一个小于等于给定 `timestamp` 的元素的下标，若下标存在，则该元素的 `value` 即为答案。
