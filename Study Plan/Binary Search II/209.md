...
### 方法一：暴力
遍历 `nums`，固定 `i`，让 `j` 从 `[i, nums.length]` 枚举，`sum` 保存 `nums[i] ... nums[j]` 的和 `sum`，若 `sum >= target`，则更新结果。
...
遍历 `nums`，固定 `i`，在区间 `[i, preSumArr.length - 1]` 内二分查找第一个满足条件（`preSumArr[mid] - preSumArr[i - 1] >= target`）的元素的下标，该区间长度为 `left - i + 1`。
