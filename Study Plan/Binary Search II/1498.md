...
### 方法一：二分查找
对 $nums$ 进行升序排序，遍历 $nums$，在区间 $[i, nums.length - 1]$ 内二分查找最后一个满足条件（$nums[i] + nums[mid] \leq target$）的元素的下标， 区间 $[i, right]$ 即为满足条件的子数组，其以 $nums[i]$ 为最小元素的子序列数目为 $2^{right - i}$，使用快速幂计算。

```Java
class Solution {
    public long quickPow(long base, long exp) {
        long ret = 1;
        while (exp > 0) {
            if ((exp & 1) == 1) {
