...
### 方法一：二分查找
在区间 `[1, maxSum]` 内二分查找枚举 `mid`，则有界数组 `arr[index] == mid`，`index` 左侧元素从 `mid` 依次减小 `1`，若减到 `1` 后下标任大于 `0`，则剩余元素全为 `1`；`index` 右侧元素从 `mid` 依次减小 `1`，若减到 `1` 后下标任小于 `n - 1`，则剩余元素全为 `1`。

统计在下标为 `index` 处以 `mid` 最大值的当前 `arr` 的数组和。二分查找最后一个满足条件（getSum(mid, index) + getSum(mid, n - 1 - index) + mid <= maxSum）的元素。

```Java
long getSum(long mid, int cnt) {
    //arr[cnt] == mid，有 cnt 项
    if (mid > cnt) {
