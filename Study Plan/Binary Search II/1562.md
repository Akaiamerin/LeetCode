...
### 方法一：模拟
题目等价于逆序遍历 `arr`，假设 `arr[i] == 0`，在 `set` 中找到距离当前最近的两侧的 `0` 的下标（左侧为 `set` 中最后一个小于 `arr[i]` 的元素，右侧为第一个大于 `arr[i]` 的元素），判断它们各自与 `arr[i]` 之间 `1` 的个数是否为 `m`，若条件成立，则返回答案，否则将 `arr[i]` 存入 `set` 中。
