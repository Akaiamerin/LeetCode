...
### 方法一：二分查找
在区间 `[0, mat.length - 1]` 内二分查找枚举 `mid`，分别获取当前行 `mid`、前一行 `mid - 1`、后一行 `mid + 1` 的最大值 `curElem`、`prevElem`，`nextElem`。

若 `curElem >= prevElem && curElem >= nextElem`，则说明当前坐标为 `mat` 中的一个峰值。
...
```Java
class Solution {
    int rowMaxElemIndex(int[][] mat, int row) {
