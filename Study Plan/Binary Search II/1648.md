...
若当前销售价值为 $mid$ 且 $inventory[i] > mid$，则此颜色球可以售出 $inventory[i] - mid$ 个。当遍历 $inventory$ 结束后，若 $sum > orders$，则可以增大 $mid$ 直到 $sum = orders$。

在区间 $[0, \max(inventory[0], \cdots, inventory[n - 1])]$ 内二分查找满足条件（在 $sum > orders$ 的条件下，球的最大销售价值）的元素。

遍历 $inventory$，若 $inventory[i] > left$，当前能销售此颜色球的数量为 $\frac{(left + 1 + inventory[i]) * (inventory[i] - left)}{2}$（等差数列求和公式），剩余数量均以 $left$ 销售。
