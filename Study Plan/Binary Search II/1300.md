...
### 方法一：二分查找
在区间 $[0, \max(arr[0], \cdots, arr[arr.length - 1])]$ 内二分查找枚举 $mid$，根据条件求出当前数组和。
...
若二分查找结束后未找到答案，则比较以 $left$ 为条件的目标和与以 $right$ 为条件的目标和，二者最小值即为答案。

```Java
class Solution {
    public int getSum(int[] arr, int maxElem) {
        int sum = 0;
