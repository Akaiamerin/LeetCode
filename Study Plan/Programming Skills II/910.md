...
### 方法一：排序 + 贪心
对 `nums` 按升序进行排序，得到 `nums` 当前的最大差值。

显然我们希望让小数变大，大数变小，以此让差值最小化。

遍历 `nums`，对于每个 `nums[i]`，尝试增加或减少 `k`（`nums[0], ..., nums[i]` 全部加 `K`,`nums[i + 1], ..., nums[nums.length() - 1]` 全部减 `K`），此时数组的最大值为 `nums[i] + K` 或 `nums[nums.length() - 1] - K`，最小值为 `nums[0] + K` 或 `nums[i + 1] - K`。
