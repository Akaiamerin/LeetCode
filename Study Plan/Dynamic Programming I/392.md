...
### 方法二：动态规划
维护一个大小为 $(s.length() + 1) * (t.length() + 1)$ 的数组 $dp$，$dp[i][j]$ 表示 $s$ 在区间 $[0, \ i - 1]$ 内的子字符串和 $t$ 在区间 $[0, \ j - 1]$ 内的子字符串相等子序列的长度。

若 $s$ 的子字符串的末尾字符等于 $t$ 的子字符串的末尾字符，则子序列的长度 $+1$，$dp[i][j] = dp[i - 1][j - 1] + 1$。

若 $s$ 的子字符串的末尾字符不等于 $t$ 的子字符串的末尾字符，则等价于可以删除 $t$ 的子字符串的末尾字符，比较 $t$ 的子字符串的前一个末尾字符，$dp[i][j] = dp[i][j - 1]$。
