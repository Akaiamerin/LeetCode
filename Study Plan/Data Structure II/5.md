...
更新回文子串最长长度 `maxLen`、回文子串的左下标 `left`、右下标 `right`。
...
由于事先无法确定最长回文子串的长度的奇偶性，所以每次扩展时都要取两次中心点（奇数长度以字符为中心，偶数长度以两个字符间的虚拟间隔为中心），再比较两次的扩展结果取二者中的最大值。
...
使用回文半径数组 `radius` 记录回文子串最中间的那个字符到回文子串两边的长度。
...
若想求以 `str[i]` 为回文中心的最长回文串的长度，则需要判断 `i` 与 `right` 的大小关系。若 `i >= right`，则只能暴力枚举，否则可以利用前面计算过的数据进行计算。

计算完成后若以 `str[i]` 为回文中心的最长回文串的右边界大于 `right`，则更新 `center` 和 `right`。最后更新遍历过的最长回文串中心下标 `maxCenter` 和其最长回文半径 `maxRadius`。
