...
### 方法一：两个栈
`mainStk` 模拟正常栈的逻辑，`auxStk` 维护最小元素。

进行 `push` 操作时，对于 `auxStk`，若 `auxStk` 为空，则直接将 `val` 入栈，否则取 `auxStk` 栈顶元素和 `val` 中的较小值入栈。
...
对于 `push` 操作，若栈为空，则执行 `2` 次将 `val` 入栈。否则，先获取当前最小值 `curMinVal`（栈顶元素），第一次将 `val` 入栈，第二次将 `curMinVal` 和 `val` 中的较小值入栈。这样，栈顶元素始终是当前栈中的最小元素。
