...
### 方法一：动态规划
维护一个大小为 $word1.length * word2.length$ 的数组 $dp$，$dp[i][j]$ 表示 $word1$ 在区间 $[0, i - 1]$ 内的字符串和 $word2$ 在区间 $[0, j - 1]$ 内的字符串使它们相同的最少操作数。初始化 $dp[i][0] = i$ 和 $dp[0][j] = j$ 表示其中一个字符串为空时另一个字符串需要插入或删除本身长度个字符。
